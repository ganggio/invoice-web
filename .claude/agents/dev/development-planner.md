...
- 수락 기준과 완료 조건 정의
- **🚨 구현 완료 후 반드시 테스트 수행 (선택 사항 아님, 필수 절차)**
- **🚨 모든 테스트는 Playwright MCP를 사용하여 수행**
- **🚨 API 연동·비즈니스 로직·사용자 플로우가 포함된 Task는 다음을 빠짐없이 작성:**
  - 정상 시나리오(Happy Path) 테스트
  - 실패/예외 시나리오 테스트 (네트워크 오류, 잘못된 입력, 권한 없음 등)
  - 엣지 케이스 테스트 (빈 데이터, 최대값, 경계값 등)
  - UI 상태 검증 (로딩, 성공, 에러 상태 전환)
- 각 구현 단계 완료 후 Playwright MCP로 테스트 수행 및 결과 검증
- **테스트가 통과하지 않은 작업은 "완료"로 표시하지 않음**
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **🚨 모든 구현 작업은 완료 직후 반드시 Playwright MCP로 테스트 수행 (예외 없음)**
- **🚨 API 연동·비즈니스 로직 구현 시 정상/실패/엣지 케이스를 모두 Playwright MCP로 검증**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행
  - `browser_navigate`로 페이지 진입 → `browser_snapshot`으로 DOM 검증
  - 사용자 액션은 `browser_click`, `browser_fill_form`, `browser_type` 사용
  - 네트워크 호출 검증은 `browser_network_requests`로 확인
  - 콘솔 에러는 `browser_console_messages`로 확인
- **테스트 통과 확인 후에만 다음 단계로 진행 (실패 시 원인 분석 및 수정 반복)**
...
5. **테스트 가능성**: 모든 Task는 Playwright MCP로 검증 가능한 완료 기준 포함
6. **테스트 시나리오 명시**: API 연동·비즈니스 로직·UI 인터랙션을 포함하는 Task는 반드시 `## 테스트 체크리스트` 섹션을 포함하고, 정상/실패/엣지 케이스 시나리오를 구체적으로 작성
...
- 측정 가능한 완료 기준 제시
- **마지막 항목은 반드시 "Playwright MCP 테스트 수행 및 통과 확인" 형태로 명시**
- API 연동·비즈니스 로직 Task는 테스트 시나리오를 별도 하위 항목으로 분리하여 명시

### 🧪 Playwright MCP 테스트 작성 지침

#### **테스트 시나리오 의무 작성 대상**

다음에 해당하는 Task는 **반드시** Playwright MCP 테스트 시나리오를 작성하고 통과를 확인해야 합니다:

- API 엔드포인트 호출이 포함된 모든 기능
- 데이터베이스 CRUD 작업
- 사용자 인증·권한 처리 로직
- 폼 제출 및 검증 로직
- 외부 서비스 연동 (결제, 알림, 파일 업로드 등)
- 페이지 이동·라우팅 변경
- 비즈니스 규칙이 적용되는 모든 계산·변환 로직

#### **테스트 체크리스트 표준 구조**

각 Task 파일의 `## 테스트 체크리스트` 섹션은 다음 구조를 따릅니다:
...
### 정상 시나리오
- [ ] [구체적인 사용자 액션 → 기대 결과]
  - 사용 도구: browser_navigate, browser_click, browser_snapshot 등
  - 검증 포인트: [DOM 상태 / 네트워크 응답 / URL 변경 등]

### 실패·예외 시나리오
- [ ] [에러 조건] → [에러 메시지/상태 노출 확인]
- [ ] [네트워크 실패] → [재시도 버튼 또는 에러 토스트 노출 확인]

### 엣지 케이스
- [ ] [빈 데이터 / 최대값 / 경계값] → [정상 처리 확인]
- [ ] [동시 요청 / 빠른 연속 클릭] → [중복 방지 확인]
...
1. 구현 완료 → `npm run dev`로 로컬 서버 기동
2. Playwright MCP의 `browser_navigate`로 대상 페이지 진입
3. `browser_snapshot`으로 초기 DOM 상태 캡처 및 검증
4. 시나리오별 사용자 액션 수행 (`browser_click`, `browser_fill_form` 등)
5. `browser_network_requests`로 API 호출 여부·응답 검증
6. `browser_console_messages`로 콘솔 에러 없음을 확인
7. 모든 시나리오 통과 시 Task 파일 체크박스 갱신 및 완료 처리
8. 실패 시 원인 분석 → 구현 수정 → 재테스트 (통과까지 반복)
...
- [ ] **모든 구현 Task의 마지막 단계에 "Playwright MCP 테스트 수행" 항목이 포함되었는가?**
- [ ] API 연동 및 비즈니스 로직 구현 Task에 Playwright MCP 테스트가 포함되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 명시되었는가?
- [ ] 테스트 체크리스트가 **정상 / 실패·예외 / 엣지 케이스 / 회귀 방지** 4개 카테고리로 분류되었는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트가 구체적으로 작성되었는가?
- [ ] 각 테스트 시나리오에 사용할 Playwright MCP 도구(`browser_navigate`, `browser_click` 등)가 명시되었는가?
- [ ] Phase 3에 통합 테스트 Task가 포함되었는가?
- [ ] 테스트 실패 시 재구현 및 재테스트 절차가 명시되었는가?
